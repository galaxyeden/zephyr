...
static int flash_rpi_write(const struct device *dev, off_t offset, const void *data, size_t size)
{
	...
	if (offset_within_page > 0) {
		...
		memcpy(flash_ram_buffer, (uint8_t *)(CONFIG_FLASH_BASE_ADDRESS + page_offset), PAGE_SIZE);
	...
	if (size > 0) {
		...
		memcpy(flash_ram_buffer + size, (uint8_t *)(CONFIG_FLASH_BASE_ADDRESS + offset + size), PAGE_SIZE - size);
		flash_range_program(offset, flash_ram_buffer, PAGE_SIZE);
	}
